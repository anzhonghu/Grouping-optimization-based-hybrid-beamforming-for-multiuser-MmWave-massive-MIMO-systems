...
SNR_db = 10;              %信噪比的设置
SNR = 10.^(SNR_db./10);
rate_group = zeros(length(Ks), 1);
rate_zf = zeros(length(Ks), 1); 
rate_gre=zeros(length(Ks), 1); 
rate_ma=zeros(length(Ks), 1); 
for Loop = 1:length(Ks) %信噪比循环
    K = Ks(Loop, 1);
...
plot(Ks,rate_zf,'-kv');
hold on
plot(Ks ,rate_group,'-k*');
plot(Ks ,rate_gre,'--ks','MarkerSize',12);  
plot(Ks ,rate_ma,'--ko');  
grid on
xlabel('numbers of users');
