...
SNR_db = 10;              %信噪比的设置
SNR = 10.^(SNR_db./10);
rate_group = zeros(length(K_Ncl), 1);
rate_zf = zeros(length(K_Ncl), 1); 
rate_gre=zeros(length(K_Ncl), 1); 
rate_ma=zeros(length(K_Ncl), 1); 
for Loop = 1:length(K_Ncl) %信噪比循环
    N_cl = K_Ncl(Loop, 1);
    for Loop_1 = 1 : N %循环次数
        %%%%%%%%    信道的建立
                                ...
                                for vvv=1:N_cl
                                    ...
                                    for bbb=1:N_cl
                                        ...
                                        for jjj=1:nn
                                            ...
                                            rate_1(tt,1)=rate_1(tt,1)+log2(1+power_dd(jjj,1)/(rao_dd(jjj,1)+zao_dd(jjj,1)/SNR));
                                        end
                                    end
                                end
                            end
            ...
            if ind_shu(ii,1)==1 %%组内只有一个用户情况
                ...
                for iii=1:N_cl
                    ...
                    for jjj=1:nn
                        ...
                        zao_dd(jjj,1)=(norm(HHH(:,jjj),2))^2;
                        rate_1(iii,1)=rate_1(iii,1)+log2(1+power_dd(jjj,1)/(rao_dd(jjj,1)+zao_dd(jjj,1)/SNR));
                    end
                end
                [~,mm]=max(rate_1);
        ...
        for ii=1:K
            ...
            rate_group(Loop, 1)=rate_group(Loop, 1)+log2(1+power_ben(ii,1)/(rao_ben(ii,1)+zao_ben(ii,1)/SNR));
        end
        %%% ZF 算法
        W_zf = ((H'*H)\H')';
        rao_zf = zeros(K, 1);
        ...
        for ii=1:K
            ...
            rate_zf(Loop, 1)=rate_zf(Loop, 1)+log2(1+power_zf(ii,1)/(rao_zf(ii,1)+zao_zf(ii,1)/SNR));
        end
        %贪婪容量最大化算法
        ...
        for jj=1:K
            ...
            for iii=1:N_cl
                ...
                if jj==1
                    ...
                    rao_gg(jj,1)=0;
                    rate_2(iii,1)=rate_2(iii,1)+log2(1+power_gg(jj,1)/(rao_gg(jj,1)+zao_gg(jj,1)/SNR));
                else
                    for jjj=1:jj
                        ...
                        rate_2(iii,1)=rate_2(iii,1)+log2(1+power_gg(jjj,1)/(rao_gg(jjj,1)+zao_gg(jjj,1)/SNR));
                    end
                end
            end
            [~,mm]=max(rate_2);
        ...
        for jjj=1:K
            ...
            rate_gre(Loop, 1)=rate_gre(Loop, 1)+log2(1+power_gre(jjj,1)/(rao_gre(jjj,1)+zao_gre(jjj,1)/SNR));
        end
        %普通码本算法
        Wrf_ma=zeros(Nr,K);
        for kk=1:K
        ...
        for jjj=1:K
            ...
            rate_ma(Loop, 1)=rate_ma(Loop, 1)+log2(1+power_ma(jjj,1)/(rao_ma(jjj,1)+zao_ma(jjj,1)/SNR));
        end
        disp([Loop, Loop_1])
    end
...
rate_gre=rate_gre/N;
rate_ma=rate_ma/N;
plot(K_Ncl,rate_zf,'-kv');
hold on
plot(K_Ncl ,rate_group,'-k*');
plot(K_Ncl ,rate_gre,'--ks','MarkerSize',12);  
plot(K_Ncl ,rate_ma,'--ko');  
grid on
xlabel('numbers of Ncl');
